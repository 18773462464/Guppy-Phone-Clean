...
import com.guppy.phoneclean.interf.IScanCallback;
import com.guppy.phoneclean.model.JunkGroup;
import com.guppy.phoneclean.model.JunkInfo;
import com.guppy.phoneclean.task.OverallScanTask;
import com.guppy.phoneclean.task.ProcessScanTask;
import com.guppy.phoneclean.task.SysCacheScanTask;
import com.guppy.phoneclean.utils.CleanUtil;
import com.guppy.phoneclean.widget.ListHeaderView;
...
public class JunkCleanActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                switch (msg.what) {
                    ...
                    case MSG_SYS_CACHE_CLEAN_FINISH:
                        ...
                        if (bundle != null) {
                            ...
                            if (hanged) {
                                Toast.makeText(JunkCleanActivity.this, "Cleaned up successfully", Toast.LENGTH_SHORT)
                                        .show();
                            }
                        }
                        break;
                    ...
                    case MSG_OVERALL_POS:
                        mHeaderView.mProgress.setText(((JunkInfo) msg.obj).mPath);
                        mHeaderView.mSize.setText(CleanUtil.formatShortFileSize(JunkCleanActivity.this, getTotalSize()));
                        break;
        ...
        mAdapter = new BaseExpandableListAdapter() {
            ...
            public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
                ...
                /*if (holder.icon != null)
                    Glide.with(JunkCleanActivity.this).load(info.icon).into(holder.icon);*/
    ...
    private void clearAll() {
        Thread clearThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                JunkGroup group = mJunkGroups.get(JunkGroup.GROUP_LOG);
                junks.addAll(group.mChildren);
    ...
    private void checkScanFinish() {
        ...
        if (mIsProcessScanFinish && mIsSysCacheScanFinish && mIsOverallScanFinish) {
            ...
            mHeaderView.mProgress.setText("total discovery:" + totalSize);
    ...
    private void checkCleanFinish() {
        if (mIsProcessCleanFinish && mIsSysCacheCleanFinish && mIsOverallCleanFinish) {
            mHeaderView.mProgress.setText("Clean up completed.");
    ...
    private void startScan() {
        ...
        SysCacheScanTask sysCacheScanTask = new SysCacheScanTask(new IScanCallback() {
            ...
            public void onFinish(ArrayList<JunkInfo> children) {
                ...
                //Message msg1 = handler.obtainMessage(MSG_OVERALL_FINISH);
                msg.sendToTarget();
                //msg1.sendToTarget();
            }
        });
